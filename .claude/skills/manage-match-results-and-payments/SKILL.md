...
- **If "Pick a specific match"**:
  ...
  npx tsx scripts/update-special-misses.ts --list-matches
  ```
  Present the retrieved matches as interactive options using `AskUserQuestion`.
...
For EACH player in the match, sequentially ask (batch related questions into a single `AskUserQuestion` call where possible):
